...
1. dni para almacenar la cadena de caracteres introducida por el 
usuario mediante el método prompt().

2. letrasAceptadas contiene un array de las letras con las que debe finalizar el DNI.

3. numerosDNI almacenará, en caso de haber introducido algo en el prompt()
los caracteres del índice 0 al 7 del dni mediante substring(0, 8).

4. comprobarLetra es un booleano que permitirá realizar una de las tres condiciones para 
que el DNI introducido sea válido. Se establece su valor en false, para que en caso de 
que en el bucle se comprueben todas las letras del array y la introducida en la posición 8 
del DNI coincida con alguna en mayúscula o minúscula.

5. resultado será una cadena de caracteres con el objetivo de mostrar lo que
ha sucedido mediante el método document.write().   
*/
...
La comprobación que realiza es simple, en el caso de que se cumpla alguna de las tres 
condiciones pedidas (Que se pulse cancelar, que se escriba un 0 o que se introduzca un 
DNI válido), la variable resultado se inicializa con un valor adecuado que ofrezca 
información sobre lo ocurrido, entonces terminará el bucle. En caso de que por ejemplo, no 
se haya escrito nada, la variable resultado estará vacía y el bucle continuará. 

Se le pide al usuario un DNI que se almacena en la variable dni. A partir de
aquí hay varios casos: 

1. Si se pulsa la tecla 'cancelar', dni pasará a valer 'undefined' y 
se con un mensaje mediante document.write().
...
ninguna de las dos entrará al bloque else en el que: 
    ...
    valga true, entonces se almacena un mensaje con el dni en la variable resultado y termina el bucle.

    3.4 En el caso de que el valor introducido en la variable dni sea un 0, se escribirá en 
    pantalla con un mensaje que se almacena en la variable resultado mediante document.write().
*/
